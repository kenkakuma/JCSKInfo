...
import type { UpdatePostInput, PostData } from '@/lib/admin/types'
import { Language } from '@/config/site'

export default function EditPostPage({ params }: { params: { lang: Language; slug: string } }) {
  ...
  const fetchPost = async () => {
    try {
      const res = await fetch(`/api/admin/posts/${params.slug}?lang=${params.lang}`)
      if (res.ok) {
  ...
  const handleSubmit = async (asDraft: boolean) => {
    ...
    try {
      const res = await fetch(`/api/admin/posts/${params.slug}/update?lang=${params.lang}`, {
        method: 'PUT',
        headers: {
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">编辑文章</h1>
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              {originalPost?.frontmatter.title}
            </p>
          </div>
        </div>
